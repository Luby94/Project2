					...
					<div class="card m-2" id="comments-${commentList.ccomu_bno}">
						<div class="card-header">
							...
							<button type="button" id="delete-comment-btn"
								onclick="confirmCommentDelete(this)"
						...
						<div class="card-body" style="display: flex;">
							...
							<div>&nbsp;&nbsp;<span style="font-size: 20px;">${commentList.ccomu_good}</span></div>
						</div>
					</div>
				</c:forEach>
			</div>
					...
					<form action="/Comment/{ccomu_bno}/comments" method="POST">
						...
						<div class="mb-3" style="margin: 10px;">
							...
							<button type="button" class="btn btn-primary"
								style="margin: 10px;" id="comment-create-btn">댓글 작성</button>
						</div>
					</form>
				</div>
			</div>
...
//   	            console.log(com_id)
              ...
              .then(data => {   
									let isLiked = data.includes('false');
									
                 	console.log(data);
//             	  alert(message);
									
								  // 이미지 변경
					        const thumbIconEl = document.querySelector('.comment');
								  console.dir(thumbIconEl);
					        //const thumbIcon = thumbIconEl.data('comment-id');
								  //console.log(thumbIcon);
					        //([data-comment-id="${data.ccomm_id}"]);
	                    console.log(isLiked);
					        if (isLiked) {
	                    // 좋아요 취소된 경우
	                    thumbIconEl.innerHTML = '<img src="/img/empty-like.png" alt="Thumb Icon">';
	                    console.log(commentList.ccomm_id)
	                    location.reload(); // 페이지 새로고침
	                } else {
	                    // 좋아요 추가된 경우
	                	 thumbIconEl.innerHTML = '<img src="/img/full-like.png" alt="Thumb Icon">';
	                	 console.log(commentList.ccomm_id)
	                	 location.reload(); // 페이지 새로고침
	                }
	  ...
	  function displayLikeMarks() {
		 	 let ccomu_bno = parseInt(document.querySelector('#new-comment-id').value);
	      fetch('http://localhost:9090/Api/Comment/{ccomu_bno}/commentGet',{
								method : 'GET',
								headers : {
